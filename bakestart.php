...
if($results=mysqli_query($conn,$sql)){
    if($rs=mysqli_fetch_array($results)){
        $time=date('U')+25200;
        $sqlb = "update ovenplayer set bid=".$rs[bid]." , btime='".$time."'+'".$rs['baketime']."', status=1 where pname='".$id."' and oid='".$oven."'";
        mysqli_query($conn,$sqlb)or die("MySQL query error");
        $sqlc = "update player set material=material-'".$material."' where pname='".$id."'";
